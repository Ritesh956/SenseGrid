...
`v0.10-phase9`, `v1.0-phase10`, `v1.0.1-phase10`, `v1.0.2-phase10`, `v1.0.3-phase10`). Phase 9 (optional credibility layer — firmware) adds
...
| 10 — Report & submission artifacts | `v1.0-phase10`, `v1.0.1-phase10`, `v1.0.2-phase10`, `v1.0.3-phase10` | `docs/phase10-report.md`: architecture diagram with a live-Jaeger-trace-confirmed per-hop latency budget, latency baseline vs. under load, the scaling/saturation curve, a failure-recovery table (broker restart, processor kill, DB pause, network partition — all effectively zero data loss), the Phase 8 backup/restore RTO recapped, a design-decisions write-up, and an honest scope statement. Ran three chaos drills live for the first time (`kill_broker.sh`/`kill_processor.sh`/`pause_db.sh`, only `ramp.sh` had run before) and found/fixed a real measurement bug in `test/chaos/lib.sh`'s seq-gap check along the way (a stale reused device_id from the Phase 7 ramp test produced a bogus 43,736-message "loss" reading before a time-scoped fix). See "Report & submission artifacts (Phase 10)" above. `v1.0.1-phase10` is a follow-up patch tag (not a new phase): bumped `.github/workflows/ci.yml`'s actions to their `node24`-native majors (fixing a Node 20 deprecation warning), which changed which golangci-lint major `version: latest` + `install-mode: goinstall` resolves to (v1.64.8 → v2.13.2) and surfaced 5 real unchecked-`Close()`-error findings, fixed with the same `defer func() { _ = x.Close() }()` pattern already used for `shutdownTracing` in `cmd/control/main.go`. `v1.0.2-phase10` is a second follow-up patch: added a top-level `README.md` (previously missing entirely) with the architecture, quickstart, and a full theory/mechanics/application write-up, committed the P10 report's chart PNGs (they were gitignored as "reproducible run output," making the report's embedded images broken links on GitHub), and fixed a real console bug — the sidebar nav rendering a sibling route's stale prefetched content on click, found by manually clicking through the console in a browser after the report was already done. See "Console (Phase 5)" above for the nav bug and the related stale-fleet-devices performance finding. `v1.0.3-phase10` is a third follow-up patch, from a full verification pass over the v1.0.2 tree (build/vet/gofmt/test/lint/govulncheck/npm-audit/tsc/compose-config/doc-links) — the tree was healthy, but three real issues surfaced, all time-driven rather than code regressions. `govulncheck` reported **8 reachable stdlib CVEs**, every one from the toolchain rather than any dependency: `go.mod`'s `go 1.26.0` is only a *floor*, so a machine sitting on an older 1.26.x patch builds against that patch's stdlib — the identical finding Phase 8 fixed by bumping the toolchain, returned purely through the passage of time as fixes landed in 1.26.5/1.26.6 while the dev machine stayed on 1.26.4. Fixed with a `toolchain go1.26.8` directive (8 reachable → 0), which is reproducible for every developer *and* for CI, whose `go-version-file: go.mod` honors it — confirmed in the run log (`Acquiring go1.26.8`, `GOVERSION='go1.26.8'`) rather than assumed. Bump that line when govulncheck reports new stdlib findings; the remaining module-level finding (`x/crypto/openpgp`, unmaintained, `Fixed in: N/A`) is never imported. Also pinned `.github/workflows/ci.yml`'s golangci-lint to `v2.13.2` instead of `version: latest` — `latest` had already broken this workflow once (the `v1.0.1` action bump silently changed which major it resolved to, surfacing 5 findings on a commit that touched no Go code), so an unpinned linter turns every upstream release into a red build on an unchanged repo. And `web/console/jest.config.js` now sets `modulePathIgnorePatterns: ["<rootDir>/.next/"]`: `testMatch` already scoped which tests *run*, but jest-haste-map still crawled the whole rootDir, so after any `next build` the real `package.json` and `.next/standalone/package.json` both claimed the name `sensegrid-console` and every run printed a Haste collision warning. Two documentation gaps closed alongside: `__pycache__/`/`*.pyc` added to `.gitignore`, and a new gotcha recording that **`go test ./... -race` cannot run in this dev environment at all** — the race detector needs cgo, and the `gcc` on PATH is MinGW.org's 32-bit-only GCC 6.3.0 (`gcc -dumpmachine` → `mingw32`) while Go targets `amd64`, so every package fails with `cc1.exe: sorry, unimplemented: 64-bit mode not compiled in`, which reads as broken code but isn't; plain `go test ./...` passes everything and CI runs the race suite on `ubuntu-latest`. |
